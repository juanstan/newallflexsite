    ...
    <div class="row large-top-buffer" >
        <div class="col-md-6 col-centered float-none" >
            <div class="jumbotron text-center" >
                ...
                <h2>Sign up to PetConnect</h2>

                <ul class="nav nav-pills top-buffer">
                    <li class="active">{{ HTML::link('pet/', 'Pet owner') }}</li>
                    <li><a href="#vetdetails" >{{ HTML::link('vet/', 'Vet practice') }}</a></li>
                </ul>
                    ...
                    {{ Form::submit('Sign up with Facebook', array('class' => 'btn btn-lg btn-primary btn-block small-top-buffer')) }}

                    {{ Form::close() }}

                    {{ Form::open(array('url' => '/pet/login/twitter', 'method' => 'get')) }}

                    {{ Form::submit('Sign up with Twitter', array('class' => 'btn btn-lg btn-info btn-block small-top-buffer')) }}

                    {{ Form::close() }}

                    <p class="top-buffer">Already have an account? <strong>{{ HTML::link('pet/', 'Sign in') }}</strong>
